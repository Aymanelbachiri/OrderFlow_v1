        ...
        <form method="POST" action="{{ route('admin.sources.update', $source) }}" class="space-y-6">
            ...
            <div class="border-b pb-4">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="shield_domain_id" class="w-full px-3 py-2 border rounded">
                            ...
                            @foreach($shieldDomains as $shieldDomain)
                                <option value="{{ $shieldDomain->id }}" {{ old('shield_domain_id', $source->shield_domain_id) == $shieldDomain->id ? 'selected' : '' }}>
                    ...
                    <div class="flex items-center space-x-2">
                        <input id="use_shield_domain" type="checkbox" name="use_shield_domain" value="1" class="rounded" {{ old('use_shield_domain', $source->use_shield_domain) ? 'checked' : '' }}>
                        <label for="use_shield_domain" class="text-sm">Enable Shield Domain</label>
                    </div>
                    @if($source->shieldDomain)
                        <div class="bg-blue-50 border border-blue-200 rounded p-3 text-sm">
                            <p class="font-semibold text-blue-800">Current Shield Domain:</p>
                            <p class="text-blue-700">{{ $source->shieldDomain->domain }} ({{ $source->shieldDomain->template_name }})</p>
                            <p class="text-blue-600 text-xs mt-1">Status: {{ ucfirst($source->shieldDomain->status) }} | DNS: {{ $source->shieldDomain->dns_configured ? 'Configured' : 'Pending' }}</p>
                        </div>
                    @endif
                </div>
            </div>
