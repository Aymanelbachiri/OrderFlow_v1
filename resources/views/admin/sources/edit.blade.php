    ...
    <div class="bg-white rounded-xl border shadow p-6 max-w-4xl">
        ...
        <form method="POST" action="{{ route('admin.sources.update', $source) }}" class="space-y-6">
            ...
            <div class="border-b pb-4">
                ...
                <div class="space-y-4">
                    <div class="flex items-center space-x-2">
                        <input id="use_own_notify_email" type="checkbox" name="use_own_notify_email" value="1" class="rounded" {{ old('use_own_notify_email', $source->use_own_notify_email) ? 'checked' : '' }} onchange="document.getElementById('notify_email_field').classList.toggle('hidden', !this.checked)">
                        <label for="use_own_notify_email" class="text-sm">Send order notifications to a custom email for this source</label>
                    </div>
                    <div id="notify_email_field" class="{{ old('use_own_notify_email', $source->use_own_notify_email) ? '' : 'hidden' }}">
                        <label class="block text-sm font-medium mb-1">Notification Email</label>
                        <input type="email" name="notify_email" value="{{ old('notify_email', $source->notify_email) }}" class="w-full px-3 py-2 border rounded" placeholder="orders@example.com">
            ...
            <div class="border-b pb-4">
                ...
                <div>
                    ...
                    <input type="text" name="hotplayer_api_key" value="{{ old('hotplayer_api_key', $source->hotplayer_api_key) }}" class="w-full px-3 py-2 border rounded font-mono text-sm" placeholder="Enter your HotPlayer reseller API key">
