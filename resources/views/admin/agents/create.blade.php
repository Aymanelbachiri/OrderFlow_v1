...
<div class="space-y-8 max-w-3xl mx-auto">
    ...
    <div class="bg-[#F5F5F5] rounded-xl border border-[#D63613]/10 shadow-md p-6 animate-fade-in-up">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-[#201E1F]">Create Agent</h1>
                <p class="text-[#201E1F]/60 mt-1">Add a new agent and assign sources they can manage</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-[#F5F5F5] rounded-xl border border-[#D63613]/10 shadow-md p-6 animate-fade-in-up" style="animation-delay: 0.1s;">
        <form method="POST" action="{{ route('admin.agents.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-[#201E1F]/60 mb-2">Password *</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-[#201E1F]/60 mb-2">Confirm Password *</label>
            ...
            <div>
                ...
                <p class="text-xs text-[#201E1F]/40 mb-3">Select the sources this agent will be allowed to manage. They will only see orders, clients, and data from these sources.</p>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    @forelse($sources as $source)
                        <label class="flex items-center space-x-3 p-3 rounded-lg border border-gray-200 bg-white hover:border-[#D63613]/40 transition-colors cursor-pointer">
                            <input type="checkbox" name="sources[]" value="{{ $source->id }}"
                                   {{ in_array($source->id, old('sources', [])) ? 'checked' : '' }}
                    ...
                    @empty
                        <p class="col-span-full text-sm text-[#201E1F]/40">No active sources found. Create a source first.</p>
                    @endforelse
                </div>
                @error('sources')
